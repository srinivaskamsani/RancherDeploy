'''
Modlue exposes a CLI to interact with
Rancher
'''
...
def get_item_from_list(item, lst):
    '''
    Return item if item in lst, else return None
    '''
    for l in lst:
        if item == l:
            return l
...
@click.option('-u', '--username',
              help='Rancher API Username',
              required=True)
@click.option('--password',
              help='Rancher API Password',
              required=True)
@click.option('-p', '--publish',
...
@click.option('--api_version',
              help='Rancher API version',
              required=True)
@click.option('--rstack',
              help='Rancher Stack name',
              required=True)
@click.option('--rservice',
              help='Rancher Service name',
              required=True)
@click.option('--image',
              help='image name',
              required=True)
...
@click.option('-l', '-label',
              ...
              required=False,
              multiple=True)
@click.option('--healthcheck_port',
              help='Internal container port to health check',
              required=False)
@click.option('--healthcheck_method',
              help='GET/PUT/POST/HEAD etc. method to use for healthcheck',
              required=False)
@click.option('--healthcheck_path',
              help='HTTP path for health check',
              required=False)
@click.option('--memory',
              help='Container level memory lock',
              required=False)
@click.option('--memory_reservation',
              help='Used by Rancher for scheduling',
              required=False)
def deploy(**kwargs):
    '''
    Deploy a service based on the given CLI inputs
    '''
...
@click.option('--rstack',
              ...
              required=True)
@click.option('--rservice',
              help='Rancher Service name of service to be load balanced',
              required=True)
@click.option('--lb_source_port',
              help='Port being exposed by the LB',
              required=True)
@click.option('--lb_target_port',
              help=' port inside the container of service being load balanced',
              required=True)
@click.option('-l', '-label',
              help='Set meta data on a container',
              required=False,
...
def SetUpLB(**kwargs):
    '''
    Set up a load balancer based on the given CLI inputs
    '''
    configs = convert(kwargs)
    rancher_auth = (configs.username, configs.password)
    rancher = Rancher(configs.host, rancher_auth, configs.api_version)
    ...
    if configs.rservice not in services:
        raise ValueError(
            "Can't create a LB for a service does not exist. Please check.")
    ...
    target_service.create_load_balancer(
        configs.lb_source_port, configs.lb_target_port, convert_tuple_to_dict(configs.label))
...
def deletestack(**kwargs):
    '''
    Delete stack based on the given CLI input
    '''
    configs = convert(kwargs)
    rancher_auth = (configs.username, configs.password)
    rancher = Rancher(configs.host, rancher_auth, configs.api_version)
...
def deleteservice(**kwargs):
    '''
    Delete service based on the given CLI input
    '''
    configs = convert(kwargs)
    rancher_auth = (configs.username, configs.password)
    rancher = Rancher(configs.host, rancher_auth, configs.api_version)
